...
public class Factorielle extends Command {
	public Factorielle(String name, String alias, String description, String example,
	...
	public void execute(Argument args) {
		...
		if(args.arguments.length > 0){
			...
			if((number >= 0) && (number <= 1000)){
				...
				args.channel.sendMessage(BuildEmbed.errorEmbed("Sorry, negative numbers cannot be handled.").build()).queue();
			}
		} else {
			args.channel.sendMessage(BuildEmbed.errorEmbed("Usage: " + glados.prefix + "facto <positive integer>").build()).queue();
		}
	}
}
