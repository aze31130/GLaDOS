...
public class Profile extends Command {
	public Profile(String name, String alias, String description, String example,
		...
		try {
			...
			if(args.arguments.length == 0) {
				if(args.account != null) {
					profile = getProfile(args.account, args.channel.getGuild().getMemberById(args.account.id));
				}
			} else {
				...
				if(args.arguments[0].equalsIgnoreCase("server")) {
					...
					profile = getProfile(GLaDOS.getInstance().getAccount(args.arguments[0]), args.channel.getGuild().getMemberById(args.arguments[0]));
				}
			}
			...
			if(args.account == null) {
				args.channel.sendMessage(BuildEmbed.errorEmbed("Sorry, this account does not exist !").build()).queue();
			} else {
	...
	private EmbedBuilder getProfile(Account a, Member m) {
		EmbedBuilder profile = new EmbedBuilder()
				...
				.setAuthor("OurStory's profile", "https://discord.com/channels/676731153444765706/676731153444765709", "https://cdn.discordapp.com/icons/676731153444765706/078350d911e939323f8a70565a2cfc95.webp")
				.setThumbnail(m.getUser().getAvatarUrl())
				.setDescription(m.getAsMention())
				.addField(":sparkles: Level:", String.valueOf(a.level), true)
				.addField(":green_book: EXP " + Levels.getExperiencePercentage(a.level, a.experience), String.valueOf(a.experience) + " / " + Levels.getRequiredExperience(a.level), true)
				...
				.addField(":green_square: Experience:", String.valueOf(a.totalExperience), true)
